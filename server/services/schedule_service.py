...
def generate_schedule(
    ...
    *,
    ...
    Args:
        schedule_id: Identifier of the schedule to generate.
        slot_minutes: Base slot length in minutes. If omitted the scheduler will
            derive it from the students' lesson lengths. All students must share
            the same lesson length when this inference path is used.
        buffer_minutes: Optional buffer appended to each slot.
        day_open_cost: Fixed cost charged the first time a day is used.
        gap_penalty: Controls how strongly we prefer back-to-back lessons within
            an opened day. Higher values bias the solver toward earlier slots.

    Returns:
        A dictionary containing the scheduled lessons and the identifiers of
    ...
    schedule_days = _parse_schedule_days(schedule.days)
...
def _parse_schedule_days(days_raw: Optional[str]) -> Set[date]:
    if not days_raw:
        return set()
...
def _collect_student_slots(availabilities: Iterable[Availability]) -> Dict[int, Set[datetime]]:
